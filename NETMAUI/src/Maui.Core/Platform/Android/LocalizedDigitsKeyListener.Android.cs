...
#pragma warning restore 0618
			...
			int dend)
		{
			...
			for (int i = dend; i < dlen; i++)
			{
				...
				if (IsSignChar(c))
				{
					return new SpannableStringBuilder(""); // Nothing can be inserted in front of a sign character.
				}
			...
			for (int i = end - 1; i >= start; i--)
			{
				...
				if (strip)
				{
					if (end == start + 1)
					{
						return new SpannableStringBuilder(""); // Only one character, and it was stripped.
					}
					if (stripped == null)
					{
