...
namespace System.Maui.Platform
{
	internal class BorderDrawable : Drawable
	{
		...
		public override void Draw(Canvas canvas)
		{
			...
			if (!_drawOutlineWithBackground && Border.BackgroundColor == Color.Default)
				return;
		...
		public Color BackgroundColor => Border.BackgroundColor == Color.Default ? _defaultColor : Border.BackgroundColor;
		...
		public void DrawCircle(Canvas canvas, int width, int height, Action<Canvas> finishDraw)
		{
			try
			{
				var radius = (float)Border.CornerRadius;
				if (radius <= 0)
				{
					finishDraw(canvas);
					return;
				...
				var borderThickness = _convertToPixels(Border.BorderWidth);

				using (var path = new APath())
				{
					float borderWidth = _convertToPixels(Border.BorderWidth);
		...
		public void DrawOutline(Canvas canvas, int width, int height)
		{
			...
			paint.Color = Border.BorderColor.ToNative();
