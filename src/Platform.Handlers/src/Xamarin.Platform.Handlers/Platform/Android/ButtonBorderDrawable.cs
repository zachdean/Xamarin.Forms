...
namespace Xamarin.Platform
{
	internal class ButtonBorderDrawable : Drawable
	{
		...
		public ButtonBorderDrawable(Func<double, float> convertToPixels, XColor defaultColor, bool drawOutlineWithBackground)
		{
		...
		public ButtonBorderDrawable SetShadow(float dy, float dx, AColor color, float radius)
		{
			_shadowDx = dx;
			_shadowDy = dy;
			_shadowColor = color;
		...
		public ButtonBorderDrawable SetPadding(float top, float left)
		{
			_paddingTop = top;
			_paddingLeft = left;
			return this;
		...
		public void DrawCircle(Canvas canvas, int width, int height, Action<Canvas> finishDraw)
		{
			...
			{
				Console.WriteLine(nameof(ButtonBorderDrawable), $"Unable to create circle image: {ex}");
			}
